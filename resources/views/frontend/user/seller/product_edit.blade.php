            ...
            <div class="card">
                ...
                <div class="card-body">
                   ...
                   @php
                            ...
                            $get_id = App\Models\Category::where('id',$cat_id)->first();

                            $get_category = $get_id->parent_id ;
                           
                            $p_category = App\Models\Category::where('id',$get_category)->first();
                            $category = $p_category->parent_id  ;
                            
                            if ($category == 0) {
                                $sub_category = $product->category_id;
                                $category_id = App\Models\Category::where('id',$sub_category)->first();
                                $category_id = $category_id->parent_id;
                                 $sub_sub_category = '';
                            } else {
                                $sub_sub_category = $product->category_id;
                                $sub_category = App\Models\Category::where('id',$sub_sub_category)->first();
                                $sub_category = $sub_category->parent_id ;
                                $category_id =App\Models\Category::where('id',$sub_category)->first();
                                $category_id = $category_id->parent_id ;
                            }
                                    ...
                                    <select class="form-control " name="sub_category_id" id="sub_category_id"
                                        ...
                                        <option value="">{{ translate('Select Sub-Category') }}</option>
                                          @foreach (App\Models\Category::get() as $category)
                                           <option value="{{ $category->id }}" {{ $category->id == $sub_category ? 'selected': ''}} >
                                                {{ $category->getTranslation('name') }}
                                            </option>
                                          @endforeach
                                    </select>
                            ...
                            <div class="form-group row" id="category">
                                ...
                                <div class="col-md-8">
                                    <select class="form-control " name="sub_sub_category_id"
                                        id="sub_sub_category_id" data-live-search="true" >
                                        <option value="">{{ translate('Select Sub-Sub-Category') }}</option>
                                        @foreach (App\Models\Category::get() as $category)
                                           <option value="{{ $category->id }}" {{ $category->id == $sub_sub_category ? 'selected': ''}} >
                                                {{ $category->getTranslation('name') }}
                                            </option>
                                          @endforeach
                                    </select>
                     ...
                     <div class="form-group row" id="brand">
                                ...
                                <div class="col-lg-8">
                                    ...
                                    @php
                                        $brand_name = \App\Models\Brand::where('id',$product->brand_id)->first();
                                       
                                    @endphp
                                    <input type="text" name="brand_id" id="brand_id"
                                        class="form-control  js-typeahead-car_v1" placeholder="Brand Name"
                                        autocomplete="off" value="{{ $brand_name->name ?? ''  }}">
                                </div>
                    </div>
                    <div class="form-group row">
                ...
                <div class="form-group row">
                    <div class="col-lg-3">
                        ...
                        <input type="text" class="form-control" name="choice[]"
                            value="{{ \App\Models\Attribute::find($choice_option->attribute_id)->name ?? '' }}"
        ...
        <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control aiz-selectpicker" name="offer_title" id="offer_title">
                                <option value="">Choose Offer Title</option>
                                <option value="today_offer" {{ $product->offer_title == 'today_offer' ? 'active' : ''}} >{{ translate('Today Offer')}}</option>
                                  <option value="buy_one_get_one"{{ $product->offer_title == 'buy_one_get_one' ? 'active' : ''}}>{{ translate('Buy One Get One')}}</option>
                                <option value="save_more_buy_more" {{ $product->offer_title == 'save_more_buy_more' ? 'active' : ''}}>{{translate('Save More Buy More')}}</option>
...
$("#category_id").on('change', function() {
            get_subCategoryByCategory();
        })

        function get_subCategoryByCategory() {
            ...
            var APP_URL = "{{ route('get_all_sub_category_by_category_seller') }}";
            $.ajax({
                type: "GET",
                url: APP_URL,
                dataType: "JSON",
                ...
                success: function(data) {
                    ...
                    $('.aiz-selectpicker').selectpicker('render'); 
        ...
        $("#sub_category_id").on('change', function(){
            get_sub_subCategoryByCategory();
        })

        function get_sub_subCategoryByCategory() {
         ...
         $("#brand_id").autocomplete({
            ...
            select: function(event, ui) {
                console.log(ui);
                $("#brand_id").val(ui.item.label);
                //  $("#medicine_id").val(ui.item.value);
                return false;
            }
        });
