...
use App\Http\Resources\V2\OrderTrackHistoryCollection;
...
class PurchaseHistoryController extends Controller
{
    ...
    public function trackOrder(Request $request)
    {
        ...
        if ($request->has('order_code')) {
            $data = Order::where('code', $request->order_code)->first();
            if ($data != null) {
                return new OrderTrackHistoryCollection($data);
            }else{
                return response()->json([
                       'status' => false,
                       'error' => 'there is no order in this code '
                ],404);
            }
        }else{
             return response()->json([
